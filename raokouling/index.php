...
require '../config.php';
$db = new DBUtils();
$db -> instance('../db/raokouling.db3');
$word=$_GET['word']??NULL;

$count_sql="SELECT count(*) FROM [content]";
$gopage='?';
if($word){
	$count_sql.=" where title like '%".$word."%' or content like '%".$word."%'";
	$gopage='?word='.$word.'&';
}
//
...
$api_array['msg']=array("name"=>"绕口令","count"=>$count,"pageall"=>$pagecount,"page"=>$page,"keyword"=>$word);
